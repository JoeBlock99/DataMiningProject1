...
bar<-barplot(as.matrix(nac_por_dpto_todas_las_edades %>% select(-25)), beside=TRUE, las=2,ylim=c(0,70000), names.arg= labels(nac_por_dpto_todas_las_edades %>% select(-25)))
```
...
```
A continuación se verá por departamento:

```{r}
nac_edad0 <- nac_por_dpto_edades_simples %>% filter(Edad %in% (10:15))
