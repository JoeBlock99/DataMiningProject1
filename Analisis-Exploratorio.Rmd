...
## - Clasificacion de departamentos por nececidad (media, alta y muy alta)


En base al analisis exploratorio realizado se separaron las variables que consideramos aportan mayor valor a nuestro analisis sobre los factores que determinan la cantidad de educacion necesaria para cubrir. Estas mismas se utilizaron para determinar los clusters. De esta manera se eligieron los 3 grupos. 

A partir de los clusters obtenidos y los percentiles se determino el rango de nececidad para cada grupo tomando en cuenta clusters y percentiles y aproximando entre sí. 

1. Necesidad media: 0 a 1,000.
2. Necesidad alta: 1,000 a 3,000
3. Necesidad muy alta: 3,000 en adelante


```{r echo=FALSE, warning=FALSE}
library(plyr)
nac_por_depto <- nac_por_dpto %>% filter(Mes != "Todos los meses") %>% filter(Departamento.de.residencia != "Todos los departamentos") %>% select(2, 3, 6)
nac_por_depto$tipoDeNecesidad = cut(nac_por_depto$Total,c(0,1000,3000,10000), labels = c("1", "2", "3"))
```
...
nac_vivos_muertos_por_dpto <- na.omit(nac_vivos_muertos_por_dpto)

datosFTree <- subset (nac_vivos_muertos_por_dpto, select = -c(9))
...
arbolModelo<-rpart(Departamento.de.residencia~.,datosFTree,method = "class")
rpart.plot(arbolModelo)

save(train,test,arbolModelo, file = "Variables.RData")
load("Variables.RData")
dt_model<-rpart(Departamento.de.residencia~.,train,method = "class")
rpart.plot(dt_model)
...
sapply(test, class)
tabla <- table(test$prediccion,test$Departamento.de.residencia)
# cfm<-confusionMatrix(tabla)
# cfm
...
nac_por_estado_civil <- na.omit(nac_por_estado_civil)

datosFTree <- subset (nac_por_estado_civil, select = -c(7))
...
nac_depto_sexo_peso <- na.omit(nac_depto_sexo_peso)

datosFTree <- subset (nac_depto_sexo_peso, select = -c(6))
...
nac_edad_madre_departamento_municipio <- na.omit(nac_edad_madre_departamento_municipio)

datosFTree <- subset (nac_edad_madre_departamento_municipio, select = -c(13, 14))
...
nac_edad_madre_depto_area_etnia <- na.omit(nac_edad_madre_depto_area_etnia)

datosFTree <- subset (nac_edad_madre_depto_area_etnia, select = -c(13, 14))
